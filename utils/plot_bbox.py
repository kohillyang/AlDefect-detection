...
def get_chinese_font():
    ...
    output = subprocess.check_output(
        'fc-list :lang=zh', shell=True)
    output = output.decode('utf-8')
    font_path = output.split(":")[0]
    from matplotlib.font_manager import FontProperties
    myfont = FontProperties(fname=font_path, size=20)
    return myfont


def plot_bbox(img, bboxes, scores=None, labels=None, thresh=0.5,
              class_names=None, colors=None, ax=None,
              reverse_rgb=False, absolute_coordinates=True, font=None):
        ...
        if class_name or score:
            if font is not None:
                ax.text(xmin, ymin - 2,
                        ...
                        fontsize=12, color='white', fontproperties=font)
            else:
                ax.text(xmin, ymin - 2,
                        u'{:s} {:s}'.format(class_name, score),
                        bbox=dict(facecolor=colors[cls_id], alpha=0.5),
